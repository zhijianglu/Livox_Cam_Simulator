...
//    CfgParam
    ...
    assert(CfgParam.intensity_mapping_mode==1);
    gen_features();
    ...
    std::normal_distribution<double> gaussian_intensity_dis_white(CfgParam.intensity_dis_white[0], sqrt(CfgParam.intensity_dis_white[1]));
    std::normal_distribution<double> gaussian_intensity_dis_black(CfgParam.intensity_dis_black[0], sqrt(CfgParam.intensity_dis_black[1]));
    std::normal_distribution<double> gaussian_intensity_dis_edge(CfgParam.intensity_dis_edge[0], CfgParam.intensity_dis_edge[1]);
    std::normal_distribution<double> gaussian_distance_dis_white(CfgParam.distance_dis_white[0], CfgParam.distance_dis_white[1]);
    std::normal_distribution<double> gaussian_distance_dis_black(CfgParam.distance_dis_black[0], CfgParam.distance_dis_black[1]);
    std::normal_distribution<double> gaussian_distance_dis_edge(CfgParam.distance_dis_edge[0], CfgParam.distance_dis_edge[1]);
...
//        计算出板子所在框
        ...
        cv::Mat raw_img = cv::imread(img_file_path[frame_id], cv::IMREAD_GRAYSCALE);
        ...
        Board_to_Laser(Eigen::Vector2d(CfgParam.gride_B_side_length * (board_B_size.x()), 0), corner[1], frame_id);
        Board_to_Laser(Eigen::Vector2d(0, CfgParam.gride_B_side_length * (board_B_size.y())), corner[2], frame_id);
        Board_to_Laser(Eigen::Vector2d(CfgParam.gride_B_side_length * board_B_size.x(),
                                       CfgParam.gride_B_side_length * board_B_size.y()), corner[3], frame_id);
        ...
        Eigen::Vector2d x_rang(x_list[0] - ext_distance, x_list[3] + ext_distance);
        Eigen::Vector2d y_rang(y_list[0] - ext_distance, y_list[3] + ext_distance);
        Eigen::Vector2d z_rang(z_list[0] - ext_distance, z_list[3] + ext_distance);
...
//        计算出板子平面参数
        ...
        Board_to_Laser(Eigen::Vector2d(CfgParam.gride_B_side_length * (board_B_size.x() + 2) / 2, 0),
                       board_x_axis_l,
                       frame_id);
        Board_to_Laser(Eigen::Vector2d(0, CfgParam.gride_B_side_length * (board_B_size.y() + 2) / 2),
                       board_y_axis_l,
                       frame_id);
        ...
        cout << "till prepare cost:" << time_cnter.toc() << " ms \n";
        time_cnter.tic();
...
#pragma omp for
        for (int pt_id = 0; pt_id < num2sim; ++pt_id)
        {
            ...
            if (
                sim_pt.x > x_rang[0] && sim_pt.x < x_rang[1] &&
                    sim_pt.y > y_rang[0] && sim_pt.y < y_rang[1] &&
                    sim_pt.z > z_rang[0] && sim_pt.z < z_rang[1]
                )
            {
                //        todo: 开始计算颜色占比
...
#pragma omp for
            ...
            else
            {
                double diatance_val_white = gaussian_distance_dis_white(gen);
                Eigen::Vector3d P_3d(sim_pt.x, sim_pt.y, sim_pt.z);
                double rate = (P_3d.norm() + diatance_val_white) / P_3d.norm();
                sim_pt.x *= rate;
                sim_pt.y *= rate;
                sim_pt.z *= rate;

                Eigen::Vector3d std_pt_3d_c = gt_R_cl * P_l + gt_t_cl;

                Eigen::Vector2d pixel = Cam3D2Pixel<Eigen::Vector2d>(std_pt_3d_c);
                if(pixel.y()<CfgParam.cam_height && pixel.x()<CfgParam.cam_width )
                    sim_pt.intensity = 150.0 * (double) raw_img.at<uchar>(pixel.y(), pixel.x()) / 255.0;
            }
        }
        pcl::transformPointCloud(*curr_scene_raw, *curr_scene_raw, T_base.inverse());
